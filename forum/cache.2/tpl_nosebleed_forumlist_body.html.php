<?php if (!defined('IN_PHPBB')) exit; ?><table cellspacing="1" class="index_body">
	<tr class="toplines">
		<th colspan="2"><?php echo ((isset($this->_rootref['L_FORUM'])) ? $this->_rootref['L_FORUM'] : ((isset($user->lang['FORUM'])) ? $user->lang['FORUM'] : '{ FORUM }')); ?></th>
		<th><?php echo ((isset($this->_rootref['L_TOPICS'])) ? $this->_rootref['L_TOPICS'] : ((isset($user->lang['TOPICS'])) ? $user->lang['TOPICS'] : '{ TOPICS }')); ?></th>
		<th><?php echo ((isset($this->_rootref['L_POSTS'])) ? $this->_rootref['L_POSTS'] : ((isset($user->lang['POSTS'])) ? $user->lang['POSTS'] : '{ POSTS }')); ?></th>
		<th><?php echo ((isset($this->_rootref['L_LAST_POST'])) ? $this->_rootref['L_LAST_POST'] : ((isset($user->lang['LAST_POST'])) ? $user->lang['LAST_POST'] : '{ LAST_POST }')); ?></th>
	</tr>
	<?php $_forumrow_count = (isset($this->_tpldata['forumrow'])) ? sizeof($this->_tpldata['forumrow']) : 0;if ($_forumrow_count) {for ($_forumrow_i = 0; $_forumrow_i < $_forumrow_count; ++$_forumrow_i){$_forumrow_val = &$this->_tpldata['forumrow'][$_forumrow_i]; if ($_forumrow_val['S_IS_CAT']) {  ?>

			<tr> 
				<td colspan="5"><h2><a href="<?php echo $_forumrow_val['U_VIEWFORUM']; ?>"><?php echo $_forumrow_val['FORUM_NAME']; ?></a></h2></td>
			</tr>
		<?php } else if ($_forumrow_val['S_IS_LINK']) {  ?>

			<tr>
				<td class="row1 one"><?php echo $_forumrow_val['FORUM_FOLDER_IMG']; ?></td>
				<td class="row1 two"
				onmouseover="this.style.backgroundColor='#EBEBEB'; this.style.cursor='pointer'"
				onmouseout="this.style.backgroundColor=''"
				onclick="window.location.href='<?php echo $_forumrow_val['U_VIEWFORUM']; ?>'">
				<?php if ($_forumrow_val['FORUM_IMAGE']) {  ?>

					<div style="float: <?php echo (isset($this->_rootref['S_CONTENT_FLOW_BEGIN'])) ? $this->_rootref['S_CONTENT_FLOW_BEGIN'] : ''; ?>; margin-<?php echo (isset($this->_rootref['S_CONTENT_FLOW_END'])) ? $this->_rootref['S_CONTENT_FLOW_END'] : ''; ?>: 5px;"><?php echo $_forumrow_val['FORUM_IMAGE']; ?></div><div style="float: <?php echo (isset($this->_rootref['S_CONTENT_FLOW_BEGIN'])) ? $this->_rootref['S_CONTENT_FLOW_BEGIN'] : ''; ?>;">
				<?php } ?>

				<h3><a href="<?php echo $_forumrow_val['U_VIEWFORUM']; ?>"><?php echo $_forumrow_val['FORUM_NAME']; ?></a></h3>
				<?php echo $_forumrow_val['FORUM_DESC']; ?>

				<?php if ($_forumrow_val['FORUM_IMAGE']) {  ?></div><?php } ?>

				</td>
			<?php if ($_forumrow_val['CLICKS']) {  ?>

				<td class="row2 one" colspan="3" align="center"><?php echo ((isset($this->_rootref['L_REDIRECTS'])) ? $this->_rootref['L_REDIRECTS'] : ((isset($user->lang['REDIRECTS'])) ? $user->lang['REDIRECTS'] : '{ REDIRECTS }')); ?>: <?php echo $_forumrow_val['CLICKS']; ?></td>
			<?php } else { ?>

				<td class="row2" colspan="3" align="center">&nbsp;</td>
			<?php } ?>

		</tr>
	<?php } else { if ($_forumrow_val['S_NO_CAT']) {  ?>

			<tr>
				<td colspan="5"><h2><?php echo ((isset($this->_rootref['L_FORUM'])) ? $this->_rootref['L_FORUM'] : ((isset($user->lang['FORUM'])) ? $user->lang['FORUM'] : '{ FORUM }')); ?></h2></td>
			</tr>
		<?php } ?>

		<tr>
			<td class="row1 one"><?php echo $_forumrow_val['FORUM_FOLDER_IMG']; ?></td>
			<td class="row1 two"
			onmouseover="this.style.backgroundColor='#EBEBEB'; this.style.cursor='pointer'"
			onmouseout="this.style.backgroundColor=''"
			onclick="window.location.href='<?php echo $_forumrow_val['U_VIEWFORUM']; ?>'">
				<?php if ($_forumrow_val['FORUM_IMAGE']) {  ?><div style="float: <?php echo (isset($this->_rootref['S_CONTENT_FLOW_BEGIN'])) ? $this->_rootref['S_CONTENT_FLOW_BEGIN'] : ''; ?>; margin-<?php echo (isset($this->_rootref['S_CONTENT_FLOW_END'])) ? $this->_rootref['S_CONTENT_FLOW_END'] : ''; ?>: 5px;"><?php echo $_forumrow_val['FORUM_IMAGE']; ?></div><?php } ?>

				<h3><a href="<?php echo $_forumrow_val['U_VIEWFORUM']; ?>"><?php echo $_forumrow_val['FORUM_NAME']; ?></a></h3>
				<?php echo $_forumrow_val['FORUM_DESC']; ?>

				<?php if ($_forumrow_val['MODERATORS']) {  ?><br /><?php echo $_forumrow_val['L_MODERATOR_STR']; ?>: <?php echo $_forumrow_val['MODERATORS']; } else if (! $_forumrow_val['SUBFORUMS']) {  ?><br />&nbsp;<?php } if ($_forumrow_val['SUBFORUMS']) {  ?><br /><?php echo $_forumrow_val['L_SUBFORUM_STR']; ?> <?php echo $_forumrow_val['SUBFORUMS']; } ?>

			</td>
			<td class="row2 one"><?php echo $_forumrow_val['TOPICS']; ?></td>
			<td class="row2 one"><?php echo $_forumrow_val['POSTS']; ?></td>
			<td class="row2 three">
				<?php if ($_forumrow_val['LAST_POST_TIME']) {  ?>

					<p class="topicdetails"><?php echo $_forumrow_val['LAST_POST_TIME']; ?></p>
					<p class="topicdetails"><?php echo $_forumrow_val['LAST_POSTER_FULL']; ?>

		                  <?php if (! $this->_rootref['S_IS_BOT']) {  ?><a href="<?php echo $_forumrow_val['U_LAST_POST']; ?>"><?php echo (isset($this->_rootref['LAST_POST_IMG'])) ? $this->_rootref['LAST_POST_IMG'] : ''; ?></a><?php } ?>

					</p>
				<?php } else { ?>

					<p class="topicdetails"><?php echo ((isset($this->_rootref['L_NO_POSTS'])) ? $this->_rootref['L_NO_POSTS'] : ((isset($user->lang['NO_POSTS'])) ? $user->lang['NO_POSTS'] : '{ NO_POSTS }')); ?></p>
				<?php } ?>

			</td>
		</tr>
	<?php } }} else { ?>

	<tr>
		<td class="row1 one" colspan="5" align="center"><?php echo ((isset($this->_rootref['L_NO_FORUMS'])) ? $this->_rootref['L_NO_FORUMS'] : ((isset($user->lang['NO_FORUMS'])) ? $user->lang['NO_FORUMS'] : '{ NO_FORUMS }')); ?></td>
	</tr>
<?php } ?>

</table>