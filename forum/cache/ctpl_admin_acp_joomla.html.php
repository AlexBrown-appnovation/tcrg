...
<?php } 
global $phpbb_admin_path, $template;
$content_check = $phpbb_admin_path."style/rokbb3_mods/".$this->_rootref['MODE'].".html";
$content = "rokbb3_mods/".$this->_rootref['MODE'].".html";
if (file_exists($content_check)) {
$template->assign_vars(array(
		'ROKBB3_MODULE_STYLE'		=> $content,
)
	);
	}
 if ($this->_rootref['ROKBB3_MODULE_STYLE']) {  if (isset($this->_rootref['ROKBB3_MODULE_STYLE'])) { $this->_tpl_include($this->_rootref['ROKBB3_MODULE_STYLE']); } } else { ?>


<form id="acp_joomla" method="post" action="<?php echo (isset($this->_rootref['U_ACTION'])) ? $this->_rootref['U_ACTION'] : ''; ?>">
<?php $_options_count = (isset($this->_tpldata['options'])) ? sizeof($this->_tpldata['options']) : 0;if ($_options_count) {for ($_options_i = 0; $_options_i < $_options_count; ++$_options_i){$_options_val = &$this->_tpldata['options'][$_options_i]; if ($_options_val['S_LEGEND']) {  if (! $_options_val['S_FIRST_ROW']) {  ?>
	...
	<?php } else { ?>
		...
		<dl>
			...
			<dd><?php echo $_options_val['CONTENT']; ?></dd></dl>
