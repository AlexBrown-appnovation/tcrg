<?php if (!defined('IN_PHPBB')) exit; if ($this->_rootref['S_PRIVMSGS']) {  $this->_tpl_include('ucp_header.html'); } else { $this->_tpl_include('overall_header.html'); } if ($this->_rootref['S_FORUM_RULES']) {  ?>
...
<?php } if ($this->_rootref['S_SHOW_DRAFTS']) {  ?>
	...
	<?php $_draftrow_count = (isset($this->_tpldata['draftrow'])) ? sizeof($this->_tpldata['draftrow']) : 0;if ($_draftrow_count) {for ($_draftrow_i = 0; $_draftrow_i < $_draftrow_count; ++$_draftrow_i){$_draftrow_val = &$this->_tpldata['draftrow'][$_draftrow_i]; if (!($_draftrow_val['S_ROW_COUNT'] & 1)  ) {  ?><tr class="row1"><?php } else { ?><tr class="row2"><?php } ?>
		...
		<td style="padding: 4px;" align="center"><span class="gen"><a href="<?php echo $_draftrow_val['U_INSERT']; ?>"><?php echo ((isset($this->_rootref['L_LOAD_DRAFT'])) ? $this->_rootref['L_LOAD_DRAFT'] : ((isset($user->lang['LOAD_DRAFT'])) ? $user->lang['LOAD_DRAFT'] : '{ LOAD_DRAFT }')); ?></a></span></td>
	</tr>
	<?php }} ?>
...
<tr>
	...
	<td class="row2" valign="top">
		...
		<tr>
			<td valign="top" style="width: 100%;"><textarea name="message" rows="15" cols="76" tabindex="3" onselect="storeCaret(this);" onclick="storeCaret(this);" onkeyup="storeCaret(this);" onfocus="initInsertions();" style="width: 98%;"><?php echo (isset($this->_rootref['MESSAGE'])) ? $this->_rootref['MESSAGE'] : ''; ?></textarea></td>
...
<?php } if ($this->_rootref['CAPTCHA_TEMPLATE'] && $this->_rootref['S_CONFIRM_CODE']) {  if (isset($this->_rootref['CAPTCHA_TEMPLATE'])) { $this->_tpl_include($this->_rootref['CAPTCHA_TEMPLATE']); } } if ($this->_rootref['S_SHOW_ATTACH_BOX'] || $this->_rootref['S_SHOW_POLL_BOX']) {  ?>
	...
	<tr>
		<td class="cat" colspan="2" align="center">
			...
			<?php if ($this->_rootref['S_SAVE_ALLOWED']) {  ?>&nbsp; <input class="btnlite" type="submit" accesskey="k" tabindex="7" name="save" value="<?php echo ((isset($this->_rootref['L_SAVE_DRAFT'])) ? $this->_rootref['L_SAVE_DRAFT'] : ((isset($user->lang['SAVE_DRAFT'])) ? $user->lang['SAVE_DRAFT'] : '{ SAVE_DRAFT }')); ?>" /><?php } if ($this->_rootref['S_HAS_DRAFTS']) {  ?>&nbsp; <input class="btnlite" type="submit" accesskey="d" tabindex="8" name="load" value="<?php echo ((isset($this->_rootref['L_LOAD_DRAFT'])) ? $this->_rootref['L_LOAD_DRAFT'] : ((isset($user->lang['LOAD_DRAFT'])) ? $user->lang['LOAD_DRAFT'] : '{ LOAD_DRAFT }')); ?>" /><?php } ?>
...
<tr>
	<td class="cat" colspan="2" align="center"><?php echo (isset($this->_rootref['S_HIDDEN_FIELDS'])) ? $this->_rootref['S_HIDDEN_FIELDS'] : ''; ?>
		...
		<?php if (! $this->_rootref['S_SHOW_ATTACH_BOX'] && ! $this->_rootref['S_SHOW_POLL_BOX']) {  if ($this->_rootref['S_SAVE_ALLOWED']) {  ?>&nbsp; <input class="btnlite" type="submit" accesskey="k" tabindex="12" name="save" value="<?php echo ((isset($this->_rootref['L_SAVE_DRAFT'])) ? $this->_rootref['L_SAVE_DRAFT'] : ((isset($user->lang['SAVE_DRAFT'])) ? $user->lang['SAVE_DRAFT'] : '{ SAVE_DRAFT }')); ?>" /><?php } if ($this->_rootref['S_HAS_DRAFTS']) {  ?>&nbsp; <input class="btnlite" type="submit" accesskey="d" tabindex="13" name="load" value="<?php echo ((isset($this->_rootref['L_LOAD_DRAFT'])) ? $this->_rootref['L_LOAD_DRAFT'] : ((isset($user->lang['LOAD_DRAFT'])) ? $user->lang['LOAD_DRAFT'] : '{ LOAD_DRAFT }')); ?>" /><?php } } ?>
...
<?php if ($this->_rootref['S_DISPLAY_REVIEW']) {  $this->_tpl_include('posting_topic_review.html'); } if ($this->_rootref['S_DISPLAY_HISTORY']) {  $this->_tpl_include('ucp_pm_history.html'); } if ($this->_rootref['S_PRIVMSGS']) {  $this->_tpl_include('ucp_footer.html'); } else { $this->_tpl_include('breadcrumbs.html'); if ($this->_rootref['S_DISPLAY_ONLINE_LIST']) {  ?>
