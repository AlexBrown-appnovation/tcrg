		...
		<tr>
			...
			onclick="window.location.href='<?php echo $_topicrow_val['U_NEWEST_POST']; ?>'">
				...
				<?php } if ($_topicrow_val['PAGINATION']) {  ?>
					...
					<br />[ <?php echo ((isset($this->_rootref['L_GOTO_PAGE'])) ? $this->_rootref['L_GOTO_PAGE'] : ((isset($user->lang['GOTO_PAGE'])) ? $user->lang['GOTO_PAGE'] : '{ GOTO_PAGE }')); ?>: <?php echo $_topicrow_val['PAGINATION']; ?> ]
				<?php } ?>
...
<?php } if ($this->_rootref['S_NO_READ_ACCESS']) {  ?>
	...
	<?php } } if (! $this->_rootref['S_DISPLAY_ACTIVE'] && ( $this->_rootref['S_IS_POSTABLE'] || sizeof($this->_tpldata['topicrow']) )) {  ?>
		...
		<tr>
			<td colspan="<?php if ($this->_rootref['S_TOPIC_ICONS']) {  ?>7<?php } else { ?>6<?php } ?>"><h2><?php echo ((isset($this->_rootref['L_FORUM_RULES'])) ? $this->_rootref['L_FORUM_RULES'] : ((isset($user->lang['FORUM_RULES'])) ? $user->lang['FORUM_RULES'] : '{ FORUM_RULES }')); ?></h2></td>
		</tr>
		<tr>
